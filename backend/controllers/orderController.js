...
exports.createOrder = catchAsync(async (req, res, next) => {
    ...
    const { fullName, phone, country, city, street, postalCode, notes, guestEmail } = req.body;
    ...
    const { provider, paymentID, payerEmail, card, receiptUrl, guestName, paymentMethod } = req.body;
    if (!provider || !paymentID || !paymentMethod) {
...
exports.getAllOrders = catchAsync(async (req, res, next) => {
    ...
    if (!req.user || req.user.role !== 'admin') {
        if (req.user) {
            ...
            return next(new AppError('Guests must provide their email to view orders.', 400));
        }
    } else {
        // Admin filters
    ...
    if (req.query.createdAfter || req.query.createdBefore) {
        ...
        if (req.query.createdAfter) {
            ...
            if (isNaN(after)) return next(new AppError('Invalid createdAfter date.', 400));
            filter.createdAt.$gte = after;
        }
        if (req.query.createdBefore) {
            const before = new Date(req.query.createdBefore);
            if (isNaN(before)) return next(new AppError('Invalid createdBefore date.', 400));
    ...
    if (!orders.length) {
        return next(new AppError('No orders found for your filters or search.', 404));
    }
...
exports.getMyOrderById = catchAsync(async (req, res, next) => {
    ...
    if (!req.user || req.user.role !== 'admin') {
        ...
        // For guests - require guestEmail query parameter
            ...
            return next(new AppError('Please provide your email to view this order.', 400));
        }
    }
    ...
    if (!order) {
        return next(new AppError(`No order found with that ID for your account.`, 404));
    }
...
exports.cancelMyOrder = catchAsync(async (req, res, next) => {
    ...
    if (!isAdmin) {
        ...
        // For guests - require guestEmail in request body
            ...
            return next(new AppError('Please provide your email to cancel this order.', 400));
        }
    }
    ...
    if (!order) {
        return next(new AppError('No order found with this ID for your account.', 404));
    }
    ...
    if (order.status === 'shipped' && order.paymentStatus === 'paid') {
        return next(new AppError('Shipped and paid orders cannot be cancelled.', 400));
    }
...
exports.trackMyOrder = catchAsync(async (req, res, next) => {
    ...
    if (!isAdmin) {
        ...
        // For guests - require guestEmail in request body
            ...
            return next(new AppError('Please provide your email to track this order.', 400));
        }
    }
    ...
    if (order.status === 'cancelled' || order.isCancelled) {
        return next(new AppError('This order was cancelled and cannot be tracked.', 400));
    }
...
exports.generateInvoiceForOrder = catchAsync(async (req, res, next) => {
    ...
    await generateInvoice(order, req.user || { name: order.guestName, email: order.guestEmail }, invoicePath);
...
exports.rateOrderedProduct = catchAsync(async (req, res, next) => {
    ...
    const order = await Order.findOne(query).select('orderItems guestName guestEmail');
    if (!order) {
    ...
    if (existingReview) {
        return next(new AppError('You have already reviewed this product from this order.', 400));
    }
...
exports.updateOrderStatus = catchAsync(async (req, res, next) => {
    ...
    if (
        ...
        return next(new AppError('You cannot cancel an order that is already confirmed or beyond.', 400));
    }
...
exports.markOrderAsPaid = catchAsync(async (req, res, next) => {
    ...
    if (order.status === 'cancelled' || order.isCancelled) {
        return next(new AppError('This order is cancelled and cannot be marked as paid.', 400));
    }
...
exports.markOrderAsDelivered = catchAsync(async (req, res, next) => {
    ...
    if (order.status === 'cancelled' || order.isCancelled) {
        return next(new AppError('This order was cancelled and cannot be marked as delivered.', 400));
    }
...
exports.addTrackingInfo = catchAsync(async (req, res, next) => {
    ...
    if (!trackingNumber || !shippingProvider) {
        return next(new AppError('Tracking number and shipping provider are required.', 400));
    }
...
exports.getCustomerLifetimeValue = catchAsync(async (req, res, next) => {
    ...
    const clvData = customers.map(user => {
        ...
        const avgOrderValue = userOrders.length ? totalSpent / userOrders.length : 0;
        ...
        return {
            ...
            name: user.name,
            email: user.email,
            clv: Number((avgOrderValue * purchaseFrequency * estimatedLifespan).toFixed(2)),
        };
    });
...
exports.getAvgTimeBetweenOrders = catchAsync(async (req, res, next) => {
    const orders = await Order.find({ isPaid: true }).sort('createdAt').populate('user');
    ...
    const result = Object.entries(userMap).map(([userId, dates]) => {
